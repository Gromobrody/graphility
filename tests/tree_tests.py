...
class TreeIndexTests:
    ...
    def random_database_usage(
        self, db, operations, inserted={}, updated={}, with_get=True, max_key_value=1000
    ):
        def _insert():
    ...
    def test_get_all_smaller(self, tmpdir):
        ...
        for key in key_values:
            ...
            assert test["key"] == a["a"]
        assert db.count(
            db.get_many, "tree", limit=-1, end=20, inclusive_end=False
        ) == len(list(filter(lambda k: k < 20, key_values)))
        db.close()
    ...
    def test_get_all_bigger(self, tmpdir):
        ...
        for key in key_values:
            ...
            assert test["key"] == a["a"]
        assert db.count(
            db.get_many, "tree", limit=-1, start=20, inclusive_start=False
        ) == len(list(filter(lambda k: k > 20, key_values)))
        db.close()
    ...
    def test_get_all_between_inclusive(self, tmpdir):
        ...
        for key in key_values:
            ...
            test = db.get("tree", a["a"])
            assert test["key"] == a["a"]
        assert db.count(db.get_many, "tree", limit=-1, start=13, end=20) == len(
    ...
    def test_random_get_smaller(self, tmpdir):
        ...
        def count_and_check(all_smaller, end_key):
            assert len(all_smaller) == db.count(
                db.get_many, "tree", limit=-1, end=end_key, inclusive_end=False
            )
            sort_by_key(all_smaller)
            all_smaller.reverse()
            result = db.get_many(
                "tree", limit=-1, end=end_key, inclusive_end=False, with_storage=False
            )
    ...
    def test_random_get_smaller_equal(self, tmpdir):
        ...
        def count_and_check(all_smaller_equal, end_key):
            ...
            result = db.get_many("tree", limit=-1, end=end_key, with_storage=False)
            assert check_if_keys_match(result, all_smaller_equal)

        self.random_database_usage(db, 1000, inserted, updated, with_get=False)
        inserted_values = list(inserted.values())
        end_key = random.choice(inserted_values)["a"]
        all_smaller_equal = list(filter(lambda k: k["a"] <= end_key, inserted_values))
    ...
    def test_random_get_bigger(self, tmpdir):
        ...
        def count_and_check(all_bigger, start_key):
            assert len(all_bigger) == db.count(
                db.get_many, "tree", limit=-1, start=start_key, inclusive_start=False
            )
        ...
        all_bigger = list(filter(lambda k: k["a"] > start_key, inserted_values))
    ...
    def test_random_get_bigger_equal(self, tmpdir):
        ...
        def count_and_check(all_bigger_equal, start_key):
            ...
            result = db.get_many("tree", limit=-1, start=start_key, with_storage=False)
            assert check_if_keys_match(result, all_bigger_equal)

        self.random_database_usage(db, 1000, inserted, updated, with_get=False)
        inserted_values = list(inserted.values())
        start_key = random.choice(inserted_values)["a"]
        all_bigger_equal = list(filter(lambda k: k["a"] >= start_key, inserted_values))
    ...
    def test_random_get_between(self, tmpdir):
        ...
        def count_and_check(all_between, start_key, end_key):
            assert len(all_between) == db.count(
                db.get_many,
                "tree",
                limit=-1,
                start=start_key,
        ...
        all_between = list(
            filter(lambda k: k["a"] > start_key and k["a"] < end_key, inserted_values)
        )
    ...
    def test_random_get_between_inclusive(self, tmpdir):
        ...
        def count_and_check(all_between_inclusive, start_key, end_key):
            assert len(all_between_inclusive) == db.count(
                db.get_many, "tree", limit=-1, start=start_key, end=end_key
            )
            sort_by_key(all_between_inclusive)
            result = db.get_many(
                "tree", limit=-1, start=start_key, end=end_key, with_storage=False
            )
    ...
    def test_offset_in_get_many_functions(self, tmpdir):
        ...
        def count_and_check(lists, start_key, end_key):
            ...
            assert (count if count >= 0 else 0) == db.count(
                db.get_many, "tree", limit=-1, offset=offset, end=end_key
            )
            ...
            assert (count if count >= 0 else 0) == db.count(
                db.get_many, "tree", limit=-1, offset=offset, start=start_key
            )
        ...
        lists.append(
            len(list(filter(lambda k: k["a"] < end_key, inserted_values)))
        )  # all_smaller
        lists.append(
            len(list(filter(lambda k: k["a"] <= end_key, inserted_values)))
        )  # all_smaller_equal
        lists.append(
            len(list(filter(lambda k: k["a"] > start_key, inserted_values)))
        )  # all_bigger
        lists.append(
            len(list(filter(lambda k: k["a"] >= start_key, inserted_values)))
        )  # all_bigger_equal
    ...
    def test_find_key_in_one_el_leaf(self, tmpdir):
        ...
        assert 0 == db.count(db.get_many, "tree", end=1, limit=-1, inclusive_end=False)
        assert 1 == db.count(db.get_many, "tree", end=2, limit=-1, inclusive_end=False)
        with pytest.raises(RecordNotFound):
            db.get("tree", 2)
        db.close()
